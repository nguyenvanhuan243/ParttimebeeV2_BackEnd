...
jQuery(document).ready(function() {
    ...
    jQuery("#otp_phone_number").unbind('keyup change input paste').bind('keyup change input paste').keypress(function (e) {
       ...
       if (e.which != 8 && e.which != 0 && (e.which < 48 || e.which > 57)) {
        ...
        } else if(valLength == 7 || valLength == 8){ 
     ...
     jQuery("#otp_code").unbind('keyup change input paste').bind('keyup change input paste').keypress(function (e) {
       ...
       if (e.which != 8 && e.which != 0 && (e.which < 48 || e.which > 57)) {
        ...
        var otp_database = jQuery("otp_database").val();
        if(valLength>maxCount){
            el.val(el.val().substring(0,maxCount));
        } else if( valLength == 5){
          jQuery('.submit_otp').addClass('next_success');
              jQuery('button.submitotp').removeClass('submitotp');
        }
    });
     // setTimeout(function () {
          ...
          if(Math.floor(inputVal) == inputVal && $.isNumeric(inputVal))
           { 
            jQuery('.J_countdown1').countdown({
              ...
              afterEnd : function(){
                jQuery('.expire_otp').empty().html('Not received OTP code? <a href="#" onclick="post_request_otp(); is_clicked();">Click Here</a> to resend.');
...
function is_clicked(){
  jQuery('.otp_tlte').empty().html('OTP Sent');
  jQuery('.expire_otp').empty().html('Your OTP will expire in <span class="J_countdown1" data-diff="60"></span> seconds');
  jQuery('.J_countdown1').countdown({
    tmpl : '<span class="minute">%{s}</span>',
    end : '',
    afterEnd : function(){
